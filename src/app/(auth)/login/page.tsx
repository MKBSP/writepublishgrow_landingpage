...
function LoginForm() {
  ...
  function doRedirect(accessToken: string, refreshToken: string, userEmail: string) {
    const params = new URLSearchParams({ access_token: accessToken, refresh_token: refreshToken, email: userEmail });
    ...
    return (
      <div style={styles.page}>
        <div style={styles.card}>
          ...
          <p style={styles.text}>If the app didn&apos;t open, <a href="wpg://auth-callback" style={styles.link}>click here</a> or copy the link manually.</p>
        </div>
      </div>
    );
  }
      ...
      <div style={{ ...styles.card, maxWidth: 420 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={styles.fieldGroup}>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@yourdomain.com" required style={styles.input} />
          </div>
          <div style={styles.fieldGroup}>
            <label style={styles.label}>Password</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required minLength={6} style={styles.input} />
          </div>
